...
public:
    ...
    Relation evaluatePredicates(const string& names, vector<string> params) {
        Relation curr = allRelations.at(relationVectors[names]);
        map<string, int> myDict;
        vector<string> keys;
        vector<int> values;
    ...
    vector<int> updateValues(map<string, int> myDict, vector<int> values){
        for (map<string, int>::iterator key = myDict.begin(); key != myDict.end(); key++) values.push_back(key->second);
    ...
    Relation helpJake(string name){
        int k = getIndex(name);
        return allRelations.at(k);
    }

    void setRelation(Relation obj, string name){
         allRelations.at(getIndex(name)) = obj;
    }
