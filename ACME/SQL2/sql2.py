...
def prob1(db_file="students.db"):
    ...
    try:
        with sql.connect(db_file) as conn:
            cur = conn.cursor()
            #get the information if its fits the condition
...
def prob2(db_file="students.db"):
    ...
    try:
        with sql.connect(db_file) as conn:
            cur = conn.cursor()
            #get the information if its fits the condition
            values = cur.execute("SELECT SI.StudentName, MI.MajorName, SG.Grade "
                                #the first join
                                "FROM StudentInfo AS SI LEFT OUTER JOIN MajorInfo AS MI "
                                "ON SI.MajorID == MI.MajorID "
                                #the rest joins
                                "INNER JOIN CourseInfo AS CI, StudentGrades AS SG "
...
m
    ...
    try:
        with sql.connect(db_file) as conn:
            cur = conn.cursor()
            #get the information if its fits the condition
            values = cur.execute("SELECT CI.CourseName "
                                #the first join
...
def prob4(db_file="students.db"):
    ...
    try:
        with sql.connect(db_file) as conn:
            cur = conn.cursor()
            #get the information if its fits the condition
            values = cur.execute("SELECT MI.MajorName, COUNT(*) AS num_students "
                                "FROM StudentInfo AS SI LEFT OUTER JOIN MajorInfo AS MI "
                                "ON SI.MajorID == MI.MajorID "
                                "GROUP BY SI.MajorID "
...
def prob5(db_file="students.db"):
    ...
    try:
        with sql.connect(db_file) as conn:
            cur = conn.cursor()
            #get the information if its fits the condition
            values = cur.execute("SELECT StudentName, MI.MajorName "
                                #connect the student info chart and the majorinfo chart
                                "FROM StudentInfo AS SI LEFT OUTER JOIN MajorInfo AS MI "
                                #connect
                                "ON SI.MajorID==MI.MajorID "
                                "WHERE StudentName LIKE '%____C%';").fetchall()
...
def prob6(db_file="students.db"):
    ...
    try:
        with sql.connect(db_file) as conn:
            cur = conn.cursor()
            #get (StudentName, N, GPA)
            values = cur.execute("SELECT SI.StudentName, COUNT(*) AS N, AVG(CASE SG.Grade "
                                #different cases
                                "WHEN 'A+' THEN 4.0 "
                                "WHEN 'A' THEN 4.0 "
                                "WHEN 'A-' THEN 3.7 "
                                ...
                                "WHEN 'D-' THEN 0.7 "
                                "ELSE 0 END) AS GPA  "
                            #where the student grade is from
